        ...
        <div class="text company">
            ...
            @stack('due_at_input_start')
                @if (!$hideDueAt)
                    ...
                    <span class="float-right">@if(!empty($document->due_at)) @date($document->due_at) @endif</span><br><br>
                @endif
            ...
            <table class="lines">
                <thead style="background-color:{{ $backgroundColor }} !important; -webkit-print-color-adjust: exact;">
                    <tr>
                        ...
                        @stack('product_unit_th_start')
                            @if (!$hideQuantity)
                                <th class="product_unit text-white">{{trans('items.product_unit')}}</th>
                            @endif
                        @stack('product_unit_th_end')
                        @stack('meter_square_th_start')
                            @if (!$hideQuantity)
                                <th class="meter_square text-white">{{trans('items.meter_square_header')}}</th>
                            @endif
                        @stack('meter_square_th_end')
        ...
        <div class="text company">
            @foreach ($document->totals_sorted as $total)
                ...
                @else
                    ...
                    <div class="border-top-1 py-2">
                        <strong class="float-left">Due Amount</strong>
                        <span>@money($document->dueamount, $document->currency_code, true)</span>
                    ...
                    <div class="border-top-1 py-2">
                        ...
                        <span>@money($total->amount - $document->paid + $document->dueamount , $document->currency_code, true)</span> -->
                        
                        <span>@money($total->amount + $document->dueamount , $document->currency_code, true)</span>
