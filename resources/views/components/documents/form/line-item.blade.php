    ...
    <td class="border-right-0 border-bottom-0 p-0"
        ...
        <table class="w-100">
            ...
            <tr>
                ...
                @stack('product_unit_input_start')
                <td>
                    <akaunting-select
                            class="mb-0 select-product_unit"
                            style="margin-left: 1px; margin-right: -2px;"
                            :form-classes="[{'has-error': form.errors.has('items.' + index + '.product_unit') }]"
                            :icon="''"
                            :title="''"
                            :placeholder="'{{ trans('general.form.select.field', ['field' => trans_choice('items.product_unit', 1)]) }}'"
                            :name="'items.' + index + '.product_unit.999'"
                            :options='{{json_encode([ 'Ton'=>trans('items.ton'), 'MP'=>trans('items.mp'), 'Meter Square'=> trans('items.meter_square') ])}}'
                            :disabled-options="form.items[index].product_unit"
                            :value="row.product_unit"
                            @interface="row.product_unit = $event"
                            @change="onCalculateTotal()"
                            :form-error="form.errors.get('items.' + index + '.product_unit')"
                            :no-data-text="'{{ trans('general.no_data') }}'"
                            :no-matching-data-text="'{{ trans('general.no_matching_data') }}'"
                    ></akaunting-select>
                </td>
                @stack('product_unit_input_end')
                @stack('meter_square_td_start')
                <td class="pb-3 pl-0 pr-2 border-bottom-0">
                    @if (!$hideQuantity)
                        <div>
                            @stack('meter_square_input_start')
                            <input type="text"
                                   class="form-control text-center p-0"
                                   :name="'items.' + index + '.meter_square'"
                                   autocomplete="off"
                                   required="required"
                                   data-item="meter_square"
                                   v-model="row.meter_square"
                                   @input="onCalculateTotal"
                                   @change="form.errors.clear('items.' + index + '.meter_square')">

                            <div class="invalid-feedback d-block"
                                 v-if="form.errors.has('items.' + index + '.meter_square')"
                                 v-html="form.errors.get('items.' + index + '.meter_square')">
                            </div>
                            @stack('meter_square_input_end')
                        </div>
                    @endif
                </td>
