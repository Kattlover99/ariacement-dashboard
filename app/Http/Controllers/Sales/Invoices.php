...
class Invoices extends Controller
{
    ...
    public function show(Document $invoice)
    {
        ...
        $userid = $invoice->contact_id;
        //$dt = Document::invoice()->where('contact_id', $userid)->where('type', 'invoice')->where('id', '!=', $invoice->id)->collect()->sum('amount');
        
        
        $dt = DB::table('documents')->where('contact_id', $userid)->where('deleted_at', null)->where('type', 'invoice')->get()->sum('amount');
        
        
        
        $paidallamt = DB::table('transactions')->where('contact_id', $userid)->where('deleted_at', null)->get()->sum('amount');
            $amt = $dt - $paidallamt - $invoice->amount;
